...
class SerialPage(QWidget):
    ...
    def open_serial(self):
        # 关键修复：必须用 currentData()，否则可能拿到 None
    ...
    def _async_open_serial(self):
        try:
            port_name = self.port_combo.currentData()
            if not port_name:
                QMessageBox.warning(self, "错误", "端口选择无效（port_name=None），请先刷新串口列表后重新选择。")
                return
            ...
            QMessageBox.warning(
                self, "权限错误",
                f"无法打开串口（可能被其他程序占用）：\n端口={self.port_combo.currentData()}\n{repr(e)}\n\n"
                f"请关闭串口监视器/串口助手/另一个本程序实例后重试。"
            )
            self.serial_port = None
        except Exception as e:
    ...
    def process_json_buffer(self):
        """
        更稳健的 JSON 提取：支持连续粘包/拆包
        使用括号深度解析，避免简单 find('}') 造成误切
        """
