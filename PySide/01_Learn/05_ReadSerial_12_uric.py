...
class DataMonitorPage(QWidget):
    ...
    def update_data(self, data):
        ...
        # ascorbic_raw = glucose_raw          #Temp

        # 2. 应用滤波（自动适配卡尔曼/滑动平均/中值）
        ...
        ascorbic = ascorbic_filtered        #Temp
    ...
    def clear_all_data(self):
        ...
        reply = QMessageBox.question(
            ...
            QMessageBox.Yes | QMessageBox.No,  # yes/no选项
            QMessageBox.No  # 默认聚焦No，降低误触概率
        )
        ...
        # 6. 清空滤波缓存（滑动平均/中值滤波的历史数据）
        for key in self.filter_buffers.keys():
            self.filter_buffers[key].clear()

        # 7. 重置卡尔曼滤波器（避免残留历史状态影响新数据）
        if self.kalman_filters:
            for filter_obj in self.kalman_filters.values():
                filter_obj.reset()

        # 8. 清空待保存数据缓存（避免清空后自动保存旧数据）
        self.cached_data.clear()
