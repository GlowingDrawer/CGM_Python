...
class DataMonitorPage(QWidget):
    ...
    def init_ui(self):
        ...
        # 图表展示区域
        chart_group = QGroupBox("数据变化趋势")
        chart_layout = QVBoxLayout(chart_group)

        self.chart = QChart()
        self.chart.setTitle("葡萄糖监测数据")
        self.chart.legend().setVisible(True)
        self.chart.legend().setAlignment(Qt.AlignBottom)

        self.glucose_series = QLineSeries()
        self.glucose_series.setName("葡萄糖值")
        self.glucose_series.setColor(QColor(255, 0, 0))

        self.voltage_series = QLineSeries()
        self.voltage_series.setName("电压")
        self.voltage_series.setColor(QColor(0, 0, 255))

        self.chart.addSeries(self.glucose_series)
        self.chart.addSeries(self.voltage_series)

        self.time_axis = QValueAxis()
        self.time_axis.setTitleText("时间(秒)")
        self.time_axis.setMin(0)
        self.time_axis.setMax(60)

        self.value_axis = QValueAxis()
        self.value_axis.setTitleText("数值")
        self.value_axis.setMin(0)
        self.value_axis.setMax(1000)

        self.chart.addAxis(self.time_axis, Qt.AlignBottom)
        self.chart.addAxis(self.value_axis, Qt.AlignLeft)

        self.glucose_series.attachAxis(self.time_axis)
        self.glucose_series.attachAxis(self.value_axis)
        self.voltage_series.attachAxis(self.time_axis)
        self.voltage_series.attachAxis(self.value_axis)
    ...
    def update_data(self, data):
        ...
        if glucose > 0:
            self.glucose_series.append(seconds, glucose)
        if voltage > 0:
            self.voltage_series.append(seconds, voltage)

        # 调整X轴范围
        if seconds > self.time_axis.max():
            self.time_axis.setMax(seconds + 5)

        # 调整Y轴范围
        if self.glucose_series.count() > 0 or self.voltage_series.count() > 0:
            points = self.glucose_series.points() + self.voltage_series.points()
            if points:
                max_value = max(points, key=lambda p: p.y()).y()
                min_value = min(points, key=lambda p: p.y()).y()
                self.value_axis.setRange(max(0, min_value - 10), max_value + 10)
