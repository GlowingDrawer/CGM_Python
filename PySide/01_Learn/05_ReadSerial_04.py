...
class SerialPage(QWidget):
    ...
    # 在初始化图表的部分添加以下代码（找到原有图表初始化区域）
    def init_chart(self):
        self.chart = QChart()
        self.chart.setTitle("数据趋势图")
        self.chart_view = QChartView(self.chart)
        self.chart_view.setRenderHint(QPainter.Antialiasing)

        # 创建曲线系列（用于显示葡萄糖数据）
        self.glucose_series = QLineSeries()
        self.glucose_series.setName("葡萄糖")
        self.chart.addSeries(self.glucose_series)
        ...
        # 关联系列与坐标轴
        self.glucose_series.attachAxis(self.axis_x)
        self.glucose_series.attachAxis(self.axis_y)

        # 初始化纵轴范围
        self.initial_min_y = 1800
        self.initial_max_y = 1900
        self.axis_y.setRange(self.initial_min_y, self.initial_max_y)
        self.current_min_y = self.initial_min_y
        self.current_max_y = self.initial_max_y

        # 新增：数据保留相关参数
        self.max_time_window = 300  # 保留最近300秒的数据（可根据需要调整）
        self.glucose_data = []  # 存储(时间, 葡萄糖值)数据的缓存列表
        self.json_buffer = ""  # 确保已有此变量用于JSON解析缓存

        # 将图表添加到界面布局
        self.main_splitter.addWidget(self.chart_view)

    # 修改数据处理函数，添加数据缓存和时间窗口过滤
    def process_json_buffer(self):
        ...
        while True:
            ...
            try:
                ...
                # 新增：处理葡萄糖数据缓存
                self.update_glucose_data(json_data)
    ...
    # 新增：葡萄糖数据缓存更新函数
    def update_glucose_data(self, json_data):
        """更新葡萄糖数据缓存并刷新图表，只保留最近一段时间的数据"""
        # 提取时间和葡萄糖值（根据实际JSON字段调整键名）
        try:
            current_time = json_data.get("Seconds", 0)  # 假设时间字段为"Seconds"
            glucose_value = json_data.get("Glucose", 0)  # 假设葡萄糖值字段为"Glucose"

            if current_time <= 0 or glucose_value <= 0:
                return  # 过滤无效数据

            # 添加新数据到缓存
            self.glucose_data.append((current_time, glucose_value))

            # 过滤掉超出时间窗口的数据（只保留最近max_time_window秒的数据）
            cutoff_time = current_time - self.max_time_window
            self.glucose_data = [(t, g) for t, g in self.glucose_data if t >= cutoff_time]

            # 刷新图表显示
            self.refresh_glucose_chart()

        except Exception as e:
            print(f"更新葡萄糖数据失败: {e}")

    # 新增：刷新葡萄糖图表函数
    def refresh_glucose_chart(self):
        """根据缓存数据刷新葡萄糖图表"""
        if not self.glucose_data:
            return

        # 清空现有曲线数据
        self.glucose_series.clear()

        # 添加过滤后的有效数据
        for t, g in self.glucose_data:
            self.glucose_series.append(t, g)

        # 调整X轴范围（只显示最近的时间窗口）
        times = [t for t, g in self.glucose_data]
        current_min_x = min(times)
        current_max_x = max(times)

        # 确保X轴有一定的余量
        self.axis_x.setRange(current_min_x, current_max_x + 5)

        # 调整Y轴范围（自动适应当前数据范围）
        glucose_values = [g for t, g in self.glucose_data]
        current_min_y = min(glucose_values)
        current_max_y = max(glucose_values)

        # 保留一定的上下余量，避免数据紧贴坐标轴
        self.axis_y.setRange(current_min_y - 50, current_max_y + 50)
...
class DataMonitorPage(QWidget):
    ...
    def __init__(self):
        ...
        self.glucose_time_data = []  # 时间-葡萄糖数据缓存
        self.voltage_glucose_data = []  # 电压-葡萄糖数据缓存
        self.init_ui()
    ...
    def init_ui(self):
        ...
        self.voltage_chart_btn = QPushButton("电压-葡萄糖图表")
        ...
        self.data_table.setHorizontalHeaderLabels(["时间(秒)", "葡萄糖值", "电压", "接收时间"])
    ...
    def init_time_chart(self):
        ...
        self.time_chart.setTitle("时间-葡萄糖监测数据")
        ...
        self.time_glucose_series.setName("葡萄糖值")
        ...
        self.time_axis_y.setTitleText("葡萄糖值")
    ...
    def init_voltage_chart(self):
        ...
        self.voltage_chart.setTitle("电压-葡萄糖关系图")
        ...
        self.voltage_glucose_series.setName("葡萄糖值")
    ...
    def update_data(self, data):
        ...
        self.data_table.setItem(row, 0, QTableWidgetItem(str(data.get("Seconds", 0))))
        self.data_table.setItem(row, 1, QTableWidgetItem(str(data.get("Glucose", 0))))
        self.data_table.setItem(row, 2, QTableWidgetItem(str(data.get("Volt", 0))))
        self.data_table.setItem(row, 3, QTableWidgetItem(data.get("receive_time", "")))
    ...
    def update_time_chart(self):
        ...
        if self.glucose_time_data:
            ...
            min_time = min(t for t, g in self.glucose_time_data)

            # 确保X轴范围至少为max_time_data
            if max_time - min_time < self.max_time_data:
                self.time_axis_x.setRange(max(0, max_time - self.max_time_data), max_time)
            else:
                self.time_axis_x.setRange(min_time, max_time)

            # 调整Y轴范围
            max_glucose = max(g for t, g in self.glucose_time_data)
            min_glucose = min(g for t, g in self.glucose_time_data)
            self.time_axis_y.setRange(max(0, min_glucose - 100), max_glucose + 100)

    def update_voltage_chart(self):
        """更新电压-葡萄糖图表，保留历史数据"""
        self.voltage_glucose_series.clear()
        for v, g in self.voltage_glucose_data:
            self.voltage_glucose_series.append(v, g)

        # 调整坐标轴范围
        if self.voltage_glucose_data:
            max_voltage = max(v for v, g in self.voltage_glucose_data)
            min_voltage = min(v for v, g in self.voltage_glucose_data)
            self.voltage_axis_x.setRange(min_voltage - 0.1, max_voltage + 0.1)

            max_glucose = max(g for v, g in self.voltage_glucose_data)
            min_glucose = min(g for v, g in self.voltage_glucose_data)
            self.voltage_axis_y.setRange(max(0, min_glucose - 100), max_glucose + 100)
